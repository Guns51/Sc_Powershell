<#
Restart-Service -Name RtkAudioUniversalService

Get-WmiObject Win32_PnPSignedDriver | where {$_.friendlyname -like "Realtek(R) Audio"} |Disable-PnpDevice -Confirm:$false
Get-WmiObject Win32_PnPSignedDriver | where {$_.friendlyname -like "Realtek(R) Audio"} |Enable-PnpDevice -Confirm:$false

Start-Sleep -Seconds 5
\\172.18.207.1\echange\nircmd.exe mutesysvolume 0
\\172.18.207.1\echange\nircmd.exe setsysvolume 65500
(New-Object Media.SoundPlayer "\\172.18.207.1\echange\macronProjet.wav").PlaySync();
#>
# Find the Realtek Audio Legacy HDA driver integration *** Does not apply to the new UAD drivers ***
# Version 2.0 / 05-Jun-2020. (c) Dimitri Delopoulos
# 1.0:   Initial release
...
#        Added INF file needed for installation
# 2.0:   Added identification of HDA or UAD driver
...
$DriverComponents = Get-CimInstance Win32_PnPSignedDriver | where {$_.DeviceClass -EQ "SOFTWARECOMPONENT" -and $_.DriverProviderName -match "Realtek"} -ErrorAction SilentlyContinue
if ($DriverComponents.Count -gt 0) { $DriverType = "UAD" } 
else {$DriverType = "HDA"}
...
Write-Host $DriverVersion "($DriverType)" -NoNewline -ForegroundColor Green
if ($DriverType -eq "HDA") { 
    Write-Host " with " -NoNewline
    Write-Host $DriverIntegrationDescription -ForegroundColor Cyan -NoNewline
    if ($Integrations.Length -gt 1) { Write-Host " $OutputIntegrations`r" -NoNewline -ForegroundColor Cyan }
    Write-Host
    Write-Host "Proposed $DriverType driver        : " -NoNewline
    ...
    Write-Host
    Write-Host "Proposed $DriverType driver        : " -NoNewline
    Write-Host "Check in $OEM Support site or Microsoft Update Catalog" -ForegroundColor Cyan
    }
...
Realtek Driver Version: $DriverVersion ($DriverType)
Proposed $DriverType driver   : $DriverIntegrationName - $DriverIntegrationDescription
INF needed to install : $DriverInf`n" | Out-File $LogFile -Append unicode
if ($DriverType -eq "HDA") {
...
Remove-Variable HWID, CODEC_ID,CODEC, Driver, DriverVersion, DriverInf, OS, OEM, FF01, FF01Name, FF01Description, FF03, FF03Name, FF03Description, 
                ...
                DriverIntegration, DriverIntegrationDescription, DriverIntegrationName, Integrations, OutputIntegrations, DriverComponents, DriverType, 
